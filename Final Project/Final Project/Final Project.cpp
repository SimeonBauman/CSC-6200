...
struct SHA256 {
    ...
    void update(const uint8_t* data, size_t len) {
        bitlen += (unsigned long long)len * 8ULL;
        size_t i = 0;
    ...
    array<uint8_t, 32> finalize() {
        ...
        for (size_t i = 0;i < tmp.size(); i += 64) compress(&tmp[i]);
...
int main() {
    ...
    cout << "=============================\n";
    cout << "  SHA-256 Text Hasher\n";
    cout << "=============================\n\n";
    cout << "Paste your text below.\n";
    cout << "When finished,Ctrl+Z then Enter.\n";
    cout << "---------------------------------------------\n\n";
    ...
    while (cin) {
        cin.read(b.data(), CHUNK);
        streamsize n = cin.gcount();
        if (n > 0) {
            sha.update(reinterpret_cast<const uint8_t*>(b.data()), static_cast<size_t>(n));
            total += static_cast<size_t>(n);
            if (total % (1 << 20) < CHUNK) cout << "[Processed " << (total / 1024) << " KB...]\r" << std::flush;
        }
    }

    cout << "\nProcessing complete. Finalizing hash...\n\n";
    auto digest = sha.finalize();
    static const char* hex = "0123456789abcdef";
    cout << "SHA-256 digest:\n";
    for (uint8_t by : digest) {
        cout << hex[by >> 4] << hex[by & 0xF];
    }
    cout << "\n";
    cout << "\nDone\n";
    return 0;
}
// hash output: cb55a6273babac91252acc26cff6844621ba560f8c08ab60f89fdb63375e28d0
//Full Text:
/* Mark.1
